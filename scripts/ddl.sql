...
CREATE TABLE IF NOT EXISTS `Futbolistas` (
  ...
  PRIMARY KEY (`idFutbolista`, `idEquipo`, `idTipoDeJugador`),
  ...
  CONSTRAINT `fk_Futbolistas_TipoDeJugador1`
    FOREIGN KEY (`idTipoDeJugador`)
    REFERENCES `TipoDeJugador` (`idTipoDeJugador`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8mb4
...
CREATE TABLE IF NOT EXISTS `Suplente` (
  ...
  CONSTRAINT `fk_Suplente_Futbolistas1`
    FOREIGN KEY (`idFutbolista`)
    REFERENCES `Futbolistas` (`idFutbolista`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_Suplente_Plantillas1`
...
CREATE TABLE IF NOT EXISTS `Titular` (
  ...
  CONSTRAINT `fk_Titular_Futbolistas1`
    FOREIGN KEY (`idFutbolista`)
    REFERENCES `Futbolistas` (`idFutbolista`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_Titular_Plantillas1`
    FOREIGN KEY (`idPlantilla`)
    REFERENCES `Plantillas` (`idPlantilla`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
...
DELIMITER $$
USE `5to_rosita_fresita`$$
CREATE DEFINER=`5to_agbd`@`localhost` PROCEDURE `alta_usuario`(
...
DELIMITER $$
USE `5to_rosita_fresita`$$
CREATE DEFINER=`5to_agbd`@`localhost` PROCEDURE `validar_usuario`(
   IN p_Email VARCHAR(45),
   IN p_Contrasena VARCHAR(255)
)
BEGIN
